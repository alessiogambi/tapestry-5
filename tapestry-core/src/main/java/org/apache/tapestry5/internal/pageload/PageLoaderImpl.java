...
public class PageLoaderImpl implements PageLoader, InvalidationListener, ComponentAssemblerSource {
    ...
    private Map<String, Map<String, List<String>>> componentIdToMixinsToEmbeddedMixins = CollectionFactory.newConcurrentMap();
    ...
    {        
        for (String mixinClass : embeddedAssembler.getMixinClassNames())
        {
            ...
            for (String embeddedMixin : mixinModel.getEmbeddedMixinClassNames())
            {
                if(!componentIdToMixinsToEmbeddedMixins.containsKey(newElement.getCompleteId()))
                    componentIdToMixinsToEmbeddedMixins.put(newElement.getCompleteId(), new HashMap<String, List<String>>());
                if(!componentIdToMixinsToEmbeddedMixins.get(newElement.getCompleteId()).containsKey(mixinClass))
                    componentIdToMixinsToEmbeddedMixins.get(newElement.getCompleteId()).put(mixinClass, new ArrayList<String>());
                
                componentIdToMixinsToEmbeddedMixins.get(newElement.getCompleteId()).get(mixinClass).add(embeddedMixin);
            }
        }
        ...
        {
            if (componentIdToMixinsToEmbeddedMixins.containsKey(container.getCompleteId()))
            {
                for(String mixinClass : componentIdToMixinsToEmbeddedMixins.get(container.getCompleteId()).keySet())
                    for (String embeddedMixinClass : componentIdToMixinsToEmbeddedMixins.get(container.getCompleteId()).get(mixinClass))
                    {
