 ...
 * {@link org.apache.tapestry5.internal.pageload.EmbeddedComponentAssembler} to perform more static analysis, but that
 * may no longer be necessary, given the switch to shared (non-pooled) pages in 5.2.
 * <p/>
...
public class PageLoaderImpl implements PageLoader, InvalidationListener, ComponentAssemblerSource
{
     ...
     * {@link org.apache.tapestry5.internal.parser.ExtensionPointToken}s and replacing them with appropriate overrides.
     * Also validates that all embedded ids are accounted for.
     */
    ...
    private void parameter(AssemblerContext context)
    {
        ...
        context.add(new PageAssemblyAction()
        {
            public void execute(PageAssembly pageAssembly)
            {
                ...
                BlockImpl block = new BlockImpl(location, interner.format(
                        "Parameter %s of %s", parameterName, element.getCompleteId()));
                ...
                if (binder == null) { throw new UnknownValueException(
                        String.format(
                                "Component %s does not include a formal parameter '%s' (and does not support informal parameters).",
                                element.getCompleteId(), parameterName), location, null, new AvailableValues(
                                "Formal parameters", embeddedAssembler.getFormalParameterNames())); }
    ...
    private void block(AssemblerContext context)
    {
        ...
        context.add(new PageAssemblyAction()
        {
            public void execute(PageAssembly pageAssembly)
            {
                ...
                if (blockId != null) element.addBlock(blockId, block);
    ...
    private void bindAttributeAsParameter(AssemblerContext context, EmbeddedComponentAssembler embeddedAssembler)
    {
        ...
        addParameterBindingAction(
                context, embeddedAssembler, token.getName(), token.getValue(), BindingConstants.LITERAL,
                token.getLocation(), true);
    }
    ...
    private EmbeddedComponentAssembler startComponent(AssemblerContext context)
    {
        ...
        if (embeddedId == null) embeddedId = assembler.generateEmbeddedId(embeddedType);
        ...
        EmbeddedComponentAssembler embeddedAssembler = assembler.createEmbeddedAssembler(
                embeddedId, componentClassName, embeddedModel, token.getMixins(), token.getLocation());
            ...
            EmbeddedComponentModel embeddedModel)
    {
        ...
        for (String parameterName : embeddedModel.getParameterNames())
        {
            ...
            addParameterBindingAction(
                    context, embeddedAssembler, parameterName, parameterValue, BindingConstants.PROP,
    ...
    private void addParameterBindingAction(AssemblerContext context,
            ...
            final boolean ignoreUnmatchedFormal)
    {
        ...
        context.add(new PageAssemblyAction()
        {
            public void execute(PageAssembly pageAssembly)
            {
                ...
                if (binder == null)
                {
                    if (ignoreUnmatchedFormal) { return; }

                    throw new UnknownValueException(
                            String.format(
                                    "Component %s does not include a formal parameter '%s' (and does not support informal parameters).",
                                    pageAssembly.createdElement.peek().getCompleteId(), parameterName), null, null,
                ...
                Binding binding = elementFactory.newBinding(
                        parameterName, containerResources, embeddedResources, defaultBindingPrefix, parameterValue,
                        location);
            ...
            final String componentClassName)
    {
        context.add(new PageAssemblyAction()
        {
            public void execute(PageAssembly pageAssembly)
            {
                ...
                assemblerForSubcomponent.assembleEmbeddedComponent(
                        pageAssembly, embeddedAssembler, embeddedId, elementName, embeddedAssembler.getLocation());
                ...
                // And redirect any further content from this component's template to go into
